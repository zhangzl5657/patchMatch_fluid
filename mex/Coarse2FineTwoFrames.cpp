...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	if(nrhs>2)
	{
        ...
        //const int *dims=mxGetDimensions(prhs[2]); //原始语句
		const int *dims=reinterpret_cast<const int *>(mxGetDimensions(prhs[2]));//对上句的修改
	...
	//mexPrintf("alpha: %f   ratio: %f   minWidth: %d  nOuterFPIterations: %d  nInnerFPIterations: %d\n",alpha,ratio,minWidth,nOuterFPIterations,nInnerFPIterations);
