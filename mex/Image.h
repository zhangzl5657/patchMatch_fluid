...
public:
    ...
    bool LoadUV(const mxArray* image,bool IsImageScaleCovnersion=true);
	template <class T1>
	void LoadMatlabImageCore(const mxArray* image,bool IsImageScaleCovnersion=true);

    //zzl------------
    template <class T1>
    void LoadMatlabImageUV(const mxArray* image,bool IsImageScaleCovnersion=true);
    //-------------------
	template <class T1>
...
#else
	if(_strcmpi(type,"uint16")==0)
		sprintf(type,"unsigned short");
	if(_strcmpi(type,"uint32")==0)
		sprintf(type,"unsigned int");
...
void Image<T>::LoadMatlabImageCore(const mxArray *image,bool IsImageScaleCovnersion)
{
	...
	const int* imDim = reinterpret_cast<const int *>(mxGetDimensions(image));//zzl
	if(nDim==2)
...
template <class T>
template <class T1>
void Image<T>::LoadMatlabImageUV(const mxArray *image,bool IsImageScaleCovnersion)
{
	int nDim = mxGetNumberOfDimensions(image);
	const int* imDim = reinterpret_cast <const int *>(mxGetDimensions(image));//zzl
	if(nDim==2)
	...
	if(isfloat==true)
		for(int i=0;i<imHeight;i++)
			for(int j=0;j<imWidth;j++)
				for(int k=0;k<nChannels;k++)
					pData[offset++]=(double)pMatlabPlane[k*nPixels+j*imHeight+i];
	else
		for(int i=0;i<imHeight;i++)
			for(int j=0;j<imWidth;j++)
				for(int k=0;k<nChannels;k++)
...
//-------------zzl---------------------
template <class T>
bool Image<T>::LoadUV(const mxArray* matrix,bool IsImageScaleCovnersion)
{
	//colorType = RGB; // the color is RGB when we use matlab to load the image
	if(mxIsClass(matrix,"uint8"))
	{
		LoadMatlabImageUV<unsigned char>(matrix,IsImageScaleCovnersion);
		return true;
	}
	if(mxIsClass(matrix,"int8"))
	{
		LoadMatlabImageUV<char>(matrix,IsImageScaleCovnersion);
		return true;
	}
	if(mxIsClass(matrix,"int32"))
	{
		LoadMatlabImageUV<int>(matrix,IsImageScaleCovnersion);
		return true;
	}
	if(mxIsClass(matrix,"uint32"))
	{
		LoadMatlabImageUV<unsigned int>(matrix,IsImageScaleCovnersion);
		return true;
	}
	if(mxIsClass(matrix,"int16"))
	{
		LoadMatlabImageUV<short int>(matrix,IsImageScaleCovnersion);
		return true;
	}
	if(mxIsClass(matrix,"uint16"))
	{
		LoadMatlabImageUV<unsigned short int>(matrix,IsImageScaleCovnersion);
		return true;
	}
	if(mxIsClass(matrix,"single"))
	{
		LoadMatlabImageUV<float>(matrix,IsImageScaleCovnersion);
		return true;
	}
	if(mxIsClass(matrix,"double"))
	{
		LoadMatlabImageUV<double>(matrix,IsImageScaleCovnersion);
		return true;
	}
	mexErrMsgTxt("Unknown type of the image!");
	return false;
...
void Image<T>::OutputToMatlab(mxArray *&matrix) const
{
	...
	if(typeid(T) == typeid(unsigned char))
		matrix=mxCreateNumericArray(nDims, reinterpret_cast<const size_t *>(dims),mxUINT8_CLASS, mxREAL);//zzl,dims进行了强制类型转换
	if(typeid(T) == typeid(char))
		matrix=mxCreateNumericArray(nDims, reinterpret_cast<const size_t *>(dims),mxINT8_CLASS, mxREAL);//zzl
	if(typeid(T) == typeid(short int))
		matrix=mxCreateNumericArray(nDims,reinterpret_cast<const size_t *> (dims),mxINT16_CLASS, mxREAL);
	if(typeid(T) == typeid(unsigned short int))
		matrix=mxCreateNumericArray(nDims, reinterpret_cast<const size_t *>(dims),mxUINT16_CLASS, mxREAL);
	if(typeid(T) == typeid(int))
		matrix=mxCreateNumericArray(nDims, reinterpret_cast<const size_t *>(dims),mxINT32_CLASS, mxREAL);
	if(typeid(T) == typeid(unsigned int))
		matrix=mxCreateNumericArray(nDims,reinterpret_cast<const size_t *>(dims),mxUINT32_CLASS, mxREAL);
	if(typeid(T) == typeid(float))
		matrix=mxCreateNumericArray(nDims, reinterpret_cast<const size_t *>(dims),mxSINGLE_CLASS, mxREAL);
	if(typeid(T) == typeid(double))
		matrix=mxCreateNumericArray(nDims, reinterpret_cast<const size_t *>(dims),mxDOUBLE_CLASS, mxREAL);
